...
# When building a shared library, be more restrictive about which wxUSE_ options are enabled
if( wxBUILD_SHARED )
    set( LIB_TYPE_BOOL OFF )
else()
    set( LIB_TYPE_BOOL ON )
endif()
...
if( WIN32 )
    wx_option( wxUSE_DPI_AWARE_MANIFEST "DPI Awareness" "per-monitor"
        STRINGS "none" "system" "per-monitor" )
endif()
...
if( NOT APPLE )
    if( MSVC )
        # Use static runtime by default on Windows so that the executables can be run in Wine
        # without needing to load the MSVC runtime DLLs.
...
# support setting the C++ standard, present it as an option to the user
...
wx_option( wxBUILD_CXX_STANDARD "C++ standard used to build wxWidgets targets"
    ${wxCXX_STANDARD_DEFAULT} STRINGS COMPILER_DEFAULT 17 20 23 26 )
...
wx_option( wxBUILD_PIC "Enable position independent code (PIC)." ${LIB_TYPE_BOOL} )
mark_as_advanced( wxBUILD_PIC )
wx_option( wxUSE_NO_RTTI "disable RTTI support" OFF )

# STL options
wx_option( wxUSE_STD_IOSTREAM "use standard C++ streams" )
wx_option( wxUSE_STD_CONTAINERS "use standard C++ container classes" )

wx_option( wxUSE_UNICODE_UTF8 "use UTF-8 representation for strings" )
wx_dependent_option( wxUSE_UTF8_LOCALE_ONLY
    "only support UTF-8 locales in UTF-8 build" OFF "wxUSE_UNICODE_UTF8" ON )

if( NOT WIN32 )
    wx_option( wxUSE_VISIBILITY "use of ELF symbols visibility" )
endif()

wx_option( wxUSE_STD_STRING_CONV_IN_WXSTRING
    "provide implicit conversions to std::wstring and std::string in wxString" OFF )
wx_option( wxUSE_UNSAFE_WXSTRING_CONV
    "provide unsafe implicit conversions in wxString to const char* or std::string" OFF )
...
wx_option( wxUSE_LIBPNG PNG "use libpng (PNG image format)" )
wx_option( wxUSE_LIBJPEG JPEG "use libjpeg (JPEG file format)" )
wx_option( wxUSE_LIBTIFF TIFF "use libtiff (TIFF file format)" )
wx_option( wxUSE_ZLIB ZLIB "use zlib for LZW compression" )
wx_option( wxUSE_EXPAT EXPAT "use expat for XML parsing" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_REGEX PCRE2 "enable support for wxRegEx class" ${LIB_TYPE_BOOL} )

wx_option( wxUSE_LUNASVG "use lunasvg for rasterizing SVG" )
set( wxTHIRD_PARTY_LIBRARIES ${wxTHIRD_PARTY_LIBRARIES} wxUSE_LUNASVG "use lunasvg for rasterizing SVG" )
...
wx_option( wxUSE_OPENGL "use OpenGL (or Mesa)" ${LIB_TYPE_BOOL} )
...
# ---------------------------------------------------------------------------
wx_option( wxUSE_INTL "use internationalization system" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_XLOCALE "use x-locale support (requires wxLocale)" OFF )
wx_option( wxUSE_CONFIG "use wxConfig (and derived) classes" )

wx_option( wxUSE_SOCKETS "use socket/network classes" OFF )
wx_option( wxUSE_IPV6 "enable IPv6 support in wxSocket" OFF )
...
wx_option( wxUSE_IPC "use interprocess communication (wxSocket etc.)" ${LIB_TYPE_BOOL} )
...
wx_option( wxUSE_DEBUGREPORT "use wxDebugReport class" OFF )

if( APPLE )
    set( wxUSE_DIALUP_MANAGER_DEFAULT OFF )
else()
    # REVIEW: [Randalphwa - 12-30-2024] Changed to off -- who uses this anymore?
    set( wxUSE_DIALUP_MANAGER_DEFAULT OFF )
endif()
...
wx_option( wxUSE_FILESYSTEM "use virtual file systems classes" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_FONTENUM "use wxFontEnumerator class" )
wx_option( wxUSE_FONTMAP "use font encodings conversion classes" )
wx_option( wxUSE_FS_ARCHIVE "use virtual archive filesystems" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_FS_INET "use virtual HTTP/FTP filesystems" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_FS_ZIP "now replaced by fs_archive" )
wx_option( wxUSE_FSVOLUME "use wxFSVolume class" )
wx_option( wxUSE_FSWATCHER "use wxFileSystemWatcher class" OFF )
...
wx_option( wxUSE_PRINTF_POS_PARAMS "use wxVsnprintf() which supports positional parameters" OFF )
wx_option( wxUSE_SECRETSTORE "use wxSecretStore class" OFF )
wx_option( wxUSE_SNGLINST_CHECKER "use wxSingleInstanceChecker class" ${LIB_TYPE_BOOL} )
...
wx_option( wxUSE_STOPWATCH "use wxStopWatch class" ${LIB_TYPE_BOOL} )
wx_option( wxUSE_STREAMS "use wxStream etc classes" )
wx_option( wxUSE_SYSTEM_OPTIONS "use wxSystemOptions" )
wx_option( wxUSE_TARSTREAM "use wxTar streams" ${LIB_TYPE_BOOL} )
...
# REVIEW: [Randalphwa - 12-29-2024] This requires either wxArchive or wxZlib
...
wx_option( wxUSE_THREADS "use threads (required if WIN32 and wxUSE_FSWATCHER or wxUSE_JOYSTICK)" OFF )
...
if( wxUSE_GUI )
    ...
    wx_option( wxUSE_MS_HTML_HELP "use MS HTML Help (win32)" OFF )
    wx_option( wxUSE_HTML "use wxHTML sub-library" )
    wx_option( wxUSE_WXHTML_HELP "use wxHTML-based help" OFF )
    ...
    wx_option( wxUSE_LOGWINDOW "use wxLogWindow" OFF )
    wx_option( wxUSE_LOG_DIALOG "use wxLogDialog" OFF )
    ...
    wx_option( wxUSE_MEDIACTRL "use wxMediaCtrl class" ${LIB_TYPE_BOOL})
    ...
    wx_option( wxUSE_DRAG_AND_DROP "use Drag'n'Drop classes" ${LIB_TYPE_BOOL})
    ...
    if( UNIX AND NOT APPLE AND NOT CMAKE_SYSTEM_NAME STREQUAL "Linux" )
        ...
        set( wxUSE_JOYSTICK_DEFAULT OFF ) # KeyWorks: default to off
    endif()
    ...
    wx_option( wxUSE_METAFILE "use wxMetaFile" OFF )
    ...
    if( WIN32 )
        ...
        wx_option( wxUSE_ACCESSIBILITY "enable accessibility support" ${LIB_TYPE_BOOL})
        wx_option( wxUSE_ACTIVEX " enable wxActiveXContainer class (Win32 only)" OFF )
        wx_option( wxUSE_CRASHREPORT "enable wxCrashReport::Generate() to create mini dumps (Win32 only)" OFF )
        ...
        wx_option( wxUSE_POSTSCRIPT_ARCHITECTURE_IN_MSW "use PS printing in wxMSW (Win32 only)" OFF )
        wx_option( wxUSE_TASKBARICON_BALLOONS "enable wxTaskBarIcon::ShowBalloon() method (Win32 only)" OFF )
        wx_option( wxUSE_UXTHEME "enable support for Windows XP themed look (Win32 only) -- required for Dark Mode" )
        ...
        wx_option( wxUSE_WEBVIEW_IE "use wxWebView IE backend (Win32 only)" ${LIB_TYPE_BOOL})
