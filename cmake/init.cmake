...
elseif( DEFINED wxBUILD_CXX_STANDARD AND NOT wxBUILD_CXX_STANDARD STREQUAL COMPILER_DEFAULT )
    ...
    if( NOT wxHAVE_CXX17 )
        ...
        # do not support c++17, request it explicitly.
...
if( NOT wxBUILD_COMPATIBILITY STREQUAL "NONE" )
    set( WXWIN_COMPATIBILITY_3_2 OFF )
...
set( wxSETUP_HEADER_PATH ${CMAKE_CURRENT_SOURCE_DIR}/include )
...
if( wxUSE_ACTIVEX AND DEFINED wxUSE_OLE AND NOT wxUSE_OLE )
    message( WARNING "wxActiveXContainer requires wxUSE_OLE -- wxUSE_OLE enabled" )
    wx_option_force_value( wxUSE_OLE ON )
endif()

if( wxUSE_DRAG_AND_DROP AND DEFINED wxUSE_OLE AND NOT wxUSE_OLE )
    message( WARNING "wxUSE_DRAG_AND_DROP requires wxUSE_OLE -- wxUSE_OLE enabled" )
    wx_option_force_value( wxUSE_OLE ON )
endif()

if( wxUSE_ACCESSIBILITY AND DEFINED wxUSE_OLE AND NOT wxUSE_OLE )
    message( WARNING "wxUSE_ACCESSIBILITY requires wxUSE_OLE -- wxUSE_OLE enabled" )
    wx_option_force_value( wxUSE_OLE ON )
endif()

if( wxUSE_MEDIACTRL AND DEFINED wxUSE_ACTIVEX AND NOT wxUSE_ACTIVEX )
    message( WARNING "wxMediaCtl requires wxActiveXContainer -- wxUSE_ACTIVEX/wxUSE_OLE enabled" )
    wx_option_force_value( wxUSE_ACTIVEX ON )
    wx_option_force_value( wxUSE_OLE ON )
endif()

if( wxUSE_WEBVIEW AND wxUSE_WEBVIEW_IE AND DEFINED wxUSE_ACTIVEX AND NOT wxUSE_ACTIVEX )
    message( WARNING "wxWebViewIE requires wxActiveXContainer -- wxUSE_ACTIVEX/wxUSE_OLE enabled" )
    wx_option_force_value( wxUSE_ACTIVEX ON )
    wx_option_force_value( wxUSE_OLE ON )
endif()
...
if( wxUSE_ZIPSTREAM AND(NOT wxUSE_ARCHIVE_STREAMS OR NOT wxUSE_ZLIB) )
    message( WARNING "wxZip requires wxArchive or wxZlib -- wxUSE_ARCHIVE_STREAMS enabled" )
    wx_option_force_value( wxUSE_ARCHIVE_STREAMS ON )
endif()

if( wxUSE_TARSTREAM AND NOT wxUSE_ARCHIVE_STREAMS )
    message( WARNING "wxTar requires wxArchive -- wxUSE_ARCHIVE_STREAMS enabled" )
    wx_option_force_value( wxUSE_ARCHIVE_STREAMS ON )
endif()

if( wxUSE_FS_ZIP AND NOT wxUSE_FS_ARCHIVE )
    message( WARNING "wxZipFSHandler requires wxArchiveFSHandler -- wxUSE_FS_ARCHIVE enabled" )
    wx_option_force_value( wxUSE_FS_ARCHIVE ON )
endif()

if( wxUSE_FS_ARCHIVE AND(NOT wxUSE_FILESYSTEM OR NOT wxUSE_ARCHIVE_STREAMS) )
    message( WARNING "wxArchiveFSHandler requires wxArchive and wxFileSystem -- wxUSE_FILESYSTEM/wxUSE_ARCHIVE_STREAMS enabled" )
    wx_option_force_value( wxUSE_FILESYSTEM ON )
    wx_option_force_value( wxUSE_ARCHIVE_STREAMS ON )
endif()

if( wxUSE_FILESYSTEM AND(NOT wxUSE_STREAMS OR(NOT wxUSE_FILE AND NOT wxUSE_FFILE)) )
    message( WARNING "wxFileSystem requires wxStreams and wxFile or wxFFile -- wxUSE_STREAMS/wxUSE_FILE enabled" )
    wx_option_force_value( wxUSE_STREAMS ON )
    wx_option_force_value( wxUSE_FILE ON )
endif()

if( wxUSE_ARCHIVE_STREAMS AND NOT wxUSE_STREAMS )
    message( WARNING "wxArchive requires wxStreams -- wxUSE_STREAMS enabled" )
    wx_option_force_value( wxUSE_STREAMS ON )
endif()

if( wxUSE_TEXTFILE AND(NOT wxUSE_FILE OR NOT wxUSE_TEXTBUFFER) )
    message( WARNING "wxTextFile requires wxFile and wxTextBuffer -- wxUSE_TEXTBUFFER enabled" )
    wx_option_force_value( wxUSE_TEXTBUFFER ON )
endif()

if( wxUSE_MIMETYPE AND NOT wxUSE_TEXTFILE )
    message( WARNING "wxUSE_MIMETYPE requires wxTextFile -- wxUSE_TEXTFILE/wxUSE_TEXTBUFFER enabled" )
    wx_option_force_value( wxUSE_TEXTFILE ON )
    wx_option_force_value( wxUSE_TEXTBUFFER ON )
endif()

if( wxUSE_CONFIG )
    if( NOT wxUSE_TEXTFILE )
        message( WARNING "wxConfig requires wxTextFile -- wxUSE_TEXTFILE/wxUSE_TEXTBUFFER enabled" )
        wx_option_force_value( wxUSE_TEXTFILE ON )
        wx_option_force_value( wxUSE_TEXTBUFFER ON )
    endif()
...
if( wxUSE_INTL AND NOT wxUSE_FILE )
    message( WARNING "I18n code requires wxFile -- wxUSE_FILE enabled" )
    wx_option_force_value( wxUSE_FILE ON )
endif()
...
if (wxUSE_PROTOCOL_FTP AND NOT wxUSE_SOCKETS )
    message( WARNING "wxUSE_PROTOCOL_FTP requires wxUSE_SOCKETS -- wxUSE_SOCKETS enabled" )
    wx_option_force_value( wxUSE_SOCKETS ON )
endif()

if (wxUSE_SVG AND NOT wxUSE_LIBPNG)
    message( WARNING "wxUSE_SVG requires wxUSE_LIBPNG -- wxUSE_LIBPNG enabled" )
    wx_option_force_value( wxUSE_LIBPNG ON )
endif()
...
if( wxUSE_GUI )
    ...
    if( WIN32 AND wxUSE_GRAPHICS_DIRECT2D )
        # BUGBUG: [Randalphwa - 12-30-2024] For unknown reasons, CMake states that
        # check_include_file is not a valid command.
    ...
    if( wxUSE_WEBVIEW )
        ...
        if( WIN32 AND NOT wxUSE_WEBVIEW_IE AND NOT wxUSE_WEBVIEW_EDGE AND NOT wxUSE_WEBVIEW_CHROMIUM )
